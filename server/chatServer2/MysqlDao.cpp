	   ...
	   // 例如，如果存储过程设置了一个会话变量@result来存储输出结果，可以这样获取：
		...
		if (res->next()) {
			for (int i = 1; i <= res->getMetaData()->getColumnCount(); ++i) {
				std::cout << "Column " << i << ": " << res->getString(i) << std::endl;//测试
			}
			...
			std::cerr << "No result returned from stored procedure." << std::endl;
			pool_->returnConnection(std::move(con));  // 归还连接
			return -1;
		}
	}
		...
		std::cerr << ", SQLState: " << e.getSQLState() << " )RegUser" << std::endl;
...
bool MysqlDao::CheckEmail(const std::string& name, const std::string& email) {
	...
	try {
		...
		std::cerr << "SQLException: " << e.what();
		std::cerr << " (MySQL error code: " << e.getErrorCode();
		std::cerr << ", SQLState: " << e.getSQLState() << " )CheckEmail" << std::endl;
		return false;
	}
}
...
bool MysqlDao::UpdatePwd(const std::string& name, const std::string& newpwd) {
	...
	try {
		...
		std::cerr << "SQLException: " << e.what();
		std::cerr << " (MySQL error code: " << e.getErrorCode();
		std::cerr << ", SQLState: " << e.getSQLState() << " )UpdatePwd" << std::endl;
		return false;
	}
}
...
bool MysqlDao::CheckPwd(const std::string& name, const std::string& pwd, UserInfo& userInfo) {
	...
	try {
		...
		std::cerr << "SQLException: " << e.what();
		std::cerr << " (MySQL error code: " << e.getErrorCode();
		std::cerr << ", SQLState: " << e.getSQLState() << " )CheckPwd" << std::endl;
		return false;
	}
}
...
bool MysqlDao::AddFriendApply(const int& from, const int& to)
{
	...
	try {
		...
		std::cerr << "SQLException: " << e.what();
		std::cerr << " (MySQL error code: " << e.getErrorCode();
		std::cerr << ", SQLState: " << e.getSQLState() << " )AddFriendApply" << std::endl;
		return false;
	}
...
bool MysqlDao::AuthFriendApply(const int& from, const int& to) {
	...
	try {
		...
		std::cerr << "SQLException: " << e.what();
		std::cerr << " (MySQL error code: " << e.getErrorCode();
		std::cerr << ", SQLState: " << e.getSQLState() << " )AuthFriendApply" << std::endl;
		return false;
	}
...
bool MysqlDao::AddFriend(const int& from, const int& to, std::string back_name) {
	...
	try {
		...
		std::cerr << "SQLException: " << e.what();
		std::cerr << " (MySQL error code: " << e.getErrorCode();
		std::cerr << ", SQLState: " << e.getSQLState() << " )AddFriend" << std::endl;
		return false;
	}
...
std::shared_ptr<UserInfo> MysqlDao::GetUser(int uid)
{
	...
	try {
		...
		// 获取结果集的列数，检查是否包含我们需要的列
		int columnCount = res->getMetaData()->getColumnCount();
		std::cout << "Number of columns in the result set: " << columnCount << std::endl;

		// 打印所有列名（调试时用）
		for (int i = 1; i <= columnCount; ++i) {
			std::cout << "Column " << i << ": " << res->getMetaData()->getColumnName(i) << std::endl;
		}
		...
		std::cerr << "SQLState: " << e.getSQLState() << ", ErrorCode: " << e.getErrorCode() << std::endl;
		std::cerr << "Failed SQL query: SELECT * FROM user WHERE uid = " << uid << std::endl;
		return nullptr;
	}
}
...
std::shared_ptr<UserInfo> MysqlDao::GetUser(std::string name)
{
	...
	try {
		...
		std::cerr << "SQLException: " << e.what();
		std::cerr << " (MySQL error code: " << e.getErrorCode();
		std::cerr << ", SQLState: " << e.getSQLState() << " )GetUserName" << std::endl;
		return nullptr;
	}
}
...
bool MysqlDao::GetApplyList(int touid, std::vector<std::shared_ptr<ApplyInfo>>& applyList, int begin, int limit) {
	...
	try {
		...
		std::cerr << "SQLException: " << e.what();
		std::cerr << " (MySQL error code: " << e.getErrorCode();
		std::cerr << ", SQLState: " << e.getSQLState() << " )GetApplyList" << std::endl;
		return false;
	}
}
...
bool MysqlDao::GetFriendList(int self_id, std::vector<std::shared_ptr<UserInfo> >& user_info_list) {
	...
	try {
		...
		std::cerr << "SQLException: " << e.what();
		std::cerr << " (MySQL error code: " << e.getErrorCode();
		std::cerr << ", SQLState: " << e.getSQLState() << " )GetFriendList" << std::endl;
		return false;
	}
