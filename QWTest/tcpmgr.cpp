...
tcpMgr::tcpMgr():_host(""),_port(0),_b_recv_pending(false),_message_id(0),_message_len(0)
{
    ...
    QObject::connect(&_socket, &QTcpSocket::readyRead, [&]() {
        ...
        stream.setVersion(QDataStream::Qt_6_0);//设置QT版本
    ...
    //连接发送信号来发送数据
    QObject::connect(this, &tcpMgr::sig_send_data, this, &tcpMgr::slotSendData);
...
void tcpMgr::handleMsg(ReqId id, int len, QByteArray data)
{
    ...
    if(find_iter == _handlers.end()){
        qDebug()<<"not found id is "<< id <<"to handlers";
        return;
    }
    //second 传参
...
void tcpMgr::slotTcpConnect(ServerInfo si)
{
    ...
    _socket.connectToHost(si.Host, _port);//建立连接
}
void tcpMgr::slotSendData(ReqId reqId, QByteArray dataBytes)
{
    uint16_t id = reqId;
