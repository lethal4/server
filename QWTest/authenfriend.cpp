    ...
    ui(new Ui::authenFriend),_label_point(2,6)
{
    ...
    ui->back_ed->setPlaceholderText("1");
    ...
    InitTipLbs();
    //链接输入标签回车事件
    connect(ui->lb_ed, &customEdit::returnPressed, this, &authenFriend::SlotLabelEnter);
    connect(ui->lb_ed, &customEdit::textChanged, this, &authenFriend::SlotLabelTextChange);
    connect(ui->lb_ed, &customEdit::editingFinished, this, &authenFriend::SlotLabelEditFinished);
...
void authenFriend::SlotAddFirendLabelByClickTip(QString text)
{
    ...
    QFontMetrics fontMetrics(lb->font()); // 获取QLabel控件的字体信息
    //int textWidth = fontMetrics.width(lb->text()); // 获取文本的宽度//horizontalAdvance(lb->text());
    int textWidth = fontMetrics.horizontalAdvance(lb->text());
